$script:nlsBaseUrl = "https://network-location.citrixcloud.jp"
...
function Connect-NLS
{
    ...
    <#
        ...
        .EXAMPLE
            ...
            $geoServer = "ZZZZ"     #Replace with your geolocation server based on region (e.g.  "api-us.cloud.com","api-ap-s.cloud.com", "api-eu.cloud.com","api.citrixcloud.jp")  
            # Connect to Network Location Service
            Connect-NLS -clientId $clientId -clientSecret $clientSecret -customer $customer $geoServer
    ...
    param([Parameter(Mandatory=$true)][string]$clientId,
          ...
          [Parameter(Mandatory=$true)][string]$geoServer)

    try
    {
        GetBearerToken -clientId $clientId -clientSecret $clientSecret -geoServer $geoServer -ErrorAction Stop
    }
    catch
    {
...
function GetBearerToken
{
    ...
    $trustUrl="https://$geoServer/cctrustoauth2/$customer/tokens/clients"
    Write-Host $trustUrl $postHeaders $body
 
  
    try
    {
        $response = Invoke-RestMethod -Uri $trustUrl -Method POST -Body $body -Headers $postHeaders -SessionVariable script:websession
        $script:websession.Headers.Add("Authorization", "CWSAuth bearer=$($response.access_token)")
    }
    catch
    {
